...
class BookService(bookCatalogPath: String) {
    ...
    private def registerBookLoan(book: Book, user: User): BookLoan = {
        ...
        updateBookLoan(loans => loans + bookLoan)
        bookLoan
    }

    private def updateBookLoan(f: Set[BookLoan] => Set[BookLoan]): Unit =
        synchronized { bookLoans = f(bookLoans) } // synchronized evita que varios procesos ocupen la misma variable, si se mandan 2 peticiones solo toma 1
